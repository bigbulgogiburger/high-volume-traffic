...
public class ArticleReadApiTest {
    ...
    @Test
    void readTest(){
        ArticleReadResponse body = articleReadRestClient.get().uri("/v1/articles/{articleId}", 263335757986525184L)
    ...
    void readAllTest(){
        ArticleReadPageResponse response1 = articleReadRestClient.get()
                .uri("/v1/articles?boardId=%s&page=%s&pageSize=%s".formatted(1L, 1000L, 5))
                .retrieve()
    ...
    void readAllInfiniteScrollTest(){
        List<ArticleReadResponse> response1 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L,264043342234914816L))
                .retrieve()
        ...
        List<ArticleReadResponse> response2 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L,264043342234914816L))
                .retrieve()
                .body(new ParameterizedTypeReference<List<ArticleReadResponse>>() {
                });
