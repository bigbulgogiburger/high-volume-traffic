...
public interface CommentRepository extends JpaRepository<Comment,Long> {
    ...
    List<Comment> findAll(
            @Param("articleId") Long articleId,
            @Param("offset") Long parentCommentId,
            @Param("limit") Long limit
    );

    @Query(value =
            "   select count(*) from (" +
            "   select comment_id from comment where article_id = :articleId limit :limit) t " +
            "   ",
            nativeQuery = true)
    Long count(
            @Param("articleId") Long articleId,
    ...
    @Query(value =
            ...
            "   (parent_comment_id > :lastParentCommentId or (parent_comment_id = :lastParentCommentId and comment_id> :lastCommentId)" +
            "   )" +
            "   order by parent_comment_id asc ,comment_id asc limit :limit"
            ,nativeQuery = true)
    List<Comment> findAllInfiniteScroll(
